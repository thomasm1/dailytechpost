...
Feature: Create a category, post and comment, exercise filters and clean up
  Scenario:
    ...
    * def post = { did: '#("karate-" + unique)', date: '2026-09-04', author: 'Karate', title: '#("Karate post " + unique)', post: 'Isolated test content', blogcite: 'https://example.com', email: '#(userEmail)', categoryId: '#(categoryId)', wordCount: 3, durationGoal: 1 }
    ...
    * set post.title = 'Updated fixture'
    ...
    And header Authorization = authHeader
    And request { name: 'Karate', email: '#(userEmail)', body: 'Comment fixture content' }
    When method POST
    Then status 201
    ...
    * eval fixtures.push('posts/' + postId + '/comments/' + commentId)
    ...
    And header Authorization = authHeader
    And request { name: 'Karate', email: '#(userEmail)', body: 'Updated comment content' }
    When method PUT
    Then status 200
    ...
    * eval fixtures.pop()
    ...
    And header Authorization = authHeader
    And request { title: 'Attached reference', url: '#("https://example.com/" + unique)', ownerEmail: '#(userEmail)' }
    When method POST
    Then status 201
