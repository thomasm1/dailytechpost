...
SELECT *  FROM  dailytech.user_plan;  
SELECT *  FROM  dailytech.DATABASECHANGELOG;  
commit;
 
 -- ------------INSPECT URL-news per user
 
SELECT * FROM dailytech.news WHERE url = 'https://dataintensive.net/';
 -- FIND
 SELECT
  user_userid,
  TRIM(TRAILING '/' FROM REGEXP_REPLACE(LOWER(TRIM(url)), '^(https?://)?(www\\.)?', '')) AS normalized_url,
...
GROUP BY
  user_userid,
  TRIM(TRAILING '/' FROM REGEXP_REPLACE(LOWER(TRIM(url)), '^(https?://)?(www\\.)?', ''))
HAVING COUNT(*) > 1;
-- DELETE ------------
DELETE n1
FROM news n1
JOIN news n2
  ON n1.user_userid = n2.user_userid
 AND TRIM(TRAILING '/' FROM REGEXP_REPLACE(LOWER(TRIM(n1.url)), '^(https?://)?(www\\.)?', ''))
   = TRIM(TRAILING '/' FROM REGEXP_REPLACE(LOWER(TRIM(n2.url)), '^(https?://)?(www\\.)?', ''))
 AND n1.id > n2.id
WHERE n1.user_userid IS NOT NULL
  AND n1.url IS NOT NULL
  AND n2.url IS NOT NULL; 
 -- --------------------
 
  DELETE FROM dailytech.news WHERE id IN (152, 195, 123);
 --  DELETE FROM dailytech.comments WHERE id = null;
--   DELETE FROM  dailytech.weblinks WHERE id = null;
--    DELETE FROM  dailytech.news WHERE id = null;
--     DELETE   FROM dailytech.post_entity WHERE id = null;
--   DELETE FROM  dailytech.categories WHERE id = null;
--   DELETE FROM  dailytech.users_roles WHERE role_id is not null;
--  DELETE FROM  dailytech.roles WHERE id = null; 
--  DELETE FROM  dailytech.users WHERE userid = null;
--  DELETE FROM  dailytech.user_plan WHERE userid = null;
 
 INSERT INTO dailytech.users_roles (role_id, user_id) VALUES (1,139);
 
-- unique email 
-- unique URLs,   ALTER TABLE emailusers ADD CONSTRAINT uk_users_email UNIQUE (email);
 CREATE TEMPORARY TABLE temp_duplicate_users AS
 SELECT u1.userid
 FROM users u1
INNER JOIN (
  SELECT email, MIN(userid) AS min_id
    FROM users
    GROUP BY email
 HAVING COUNT(*) > 1
) u2 ON u1.email = u2.email AND u1.userid > u2.min_id;
 SELECT * FROM temp_duplicate_users;
  ALTER TABLE emailusers ADD CONSTRAINT uk_users_email UNIQUE (email);
  
  ALTER TABLE dailytech.weblinks ADD COLUMN  version INT DEFAULT 1;
-- ensure not null (use NOW() as a last resort)category_id
ALTER TABLE dailytech.weblinks ADD  COLUMN time_created DATETIME NOT NULL;
ALTER TABLE dailytech.weblinks ADD  COLUMN time_updated DATETIME NOT NULL;
UPDATE weblinks SET time_created = NOW() WHERE time_created IS NULL;
UPDATE weblinks SET time_updated = NOW() WHERE time_updated IS NULL;
 
-- enforce constraints
ALTER TABLE weblinks
  MODIFY COLUMN time_created DATETIME NOT NULL,
  MODIFY COLUMN time_updated DATETIME NOT NULL;
...
VALUES
  (10, 'TOKEN_STALKER_PRO', 'ACTIVE', '2025-01-01 00:00:00', '2025-12-31 23:59:59', 0,
  ...
  (12, 'TOKEN_STALKER_PRO', 'TRIALING', '2025-03-01 00:00:00', '2025-06-01 00:00:00', 1,
