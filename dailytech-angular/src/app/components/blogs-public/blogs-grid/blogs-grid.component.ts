...
export class BlogsGridComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  onCheckboxChange(event: any, category: string): void {
    const selectedCategories = this.categoriesForm.get(
      'selectedCategories',
    ) as UntypedFormArray;
    ...
    if (event.checked) {
      ...
      const index = selectedCategories.controls.findIndex(
        (x) => x.value === category,
      );
  ...
  fetchBlogs(): void {
    ...
    if (selectedCategories.length > 0) {
      this.blogs = this.allBlogs.filter((blog) =>
        selectedCategories.includes(blog.cat3),
      );
    } else {
      this.blogs = this.allBlogs;
    }
  ...
  calculateLocalStats(): void {
    ...
    statsMap.forEach((value, key) => {
      this.categoryStats.push({
        ...
        count: value.count,
        avgWordCount:
          value.count > 0 ? Math.round(value.totalWords / value.count) : 0,
      });
    });
  ...
  private toGridRow(blog: Blog): BlogGridRow {
    ...
    return {
      ...
      displayWordCount:
        blog.wordCount || this.calculateWordCount(blog.post || ''),
    };
  }
